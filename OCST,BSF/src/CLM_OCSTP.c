...
extern double* objSample, * ySample, * UBound, * LBound, M;
...
void MST(int N, double** cost, double** arc, int** info) {
	...
	int* in_T = create_int_vector(N);
	double* dist = create_double_vector(N);
	double** costPrime = create_double_matrix(N, N);

	//// For interdicted arcs, we allocate high cost ///
	for (i = 0; i < N - 1; i++) {
		for (j = i + 1; j < N; j++) {
			if (interdiction[i][j] == 1 || interdiction[i][j] == 1) {
				costPrime[i][j] = M;
				costPrime[j][i] = M;
			}
			else {
				costPrime[i][j] = cost[i][j];
				costPrime[j][i] = cost[j][i];
			}
		}
	}
	...
	for (i = 0; i < N; i++) {
		...
		dist[i] = costPrime[0][i];
		//}
	}	// current distances to the tree
	...
	for (iteration = 1; iteration < N; iteration++) {
		...
		for (j = 1; j < N; j++) {
			// Easy case: we have no restrictions, just check for the best one:
			if (info == NULL) {
				if (in_T[j] == NO && dist[j] > costPrime[index_min][j]) {
					/*if (interdiction[index_min][j] == 1 || interdiction[j][index_min] == 1) {
						dist[j] = M;
						pred[j] = index_min;
					}
					else {*/
					dist[j] = costPrime[index_min][j];
					pred[j] = index_min;
					//}
				}
				// Difficult case: we have some restrictions:
				...
				if (in_T[j] == NO && info[index_min][j] != NO) {
					// Check for optimality:
					if (info[index_min][j] == YES || info[j][pred[j]] == NO || (info[j][pred[j]] == NONE && dist[j] > costPrime[index_min][j])) {
						dist[j] = costPrime[index_min][j];
						pred[j] = index_min;
						//}
					}
...
void GHT(int N, double** W, double** Y) {
	int		i, j, k, kk, jj, ff, ii;
	int		NumYOpened;
	double	MinCost;
	int		indexMinCost;
	int		goIn;
	int* NotInTree = create_int_vector(N);
	double** capacity = create_double_matrix(N, N);
	double* flow = create_double_vector(N);
	int* tree = create_int_vector(N);
	int* SubTree = create_int_vector(N);
	int* SubTree2 = create_int_vector(N);
...
// compute the Gomory-Hu Tree
	...
	for (i = 0; i < N; i++) {
		Y[i][i] = 0.0;			// Main diagonal
		for (j = i + 1; j < N; j++) {
			if (tree[i] == j || tree[j] == i && (interdiction[i][j] < 0.5)) {
				Y[i][j] = Y[j][i] = 1.0;
				NumYOpened++;
			} // Edge in the tree 
			else { Y[i][j] = Y[j][i] = 0.0; } // Edge not in the tree
	...
	/// To make the obtained solution feasible which means that interdicted arcs cannot be part of the solution////
	goIn = 0;
	while (NumYOpened < N - 1) {
		for (i = 0; i < N; i++) { NotInTree[i] = 0; }
		goIn = 0;
		for (i = 0; i < N; i++) {
			for (j = i + 1; j < N; j++) {
				if (Y[i][j] > 0.5) {
		...
		for (i = 0; i < N; i++) {
			if (NotInTree[i] < 0.5) {
				goIn = 1;
				for (j = 0; j < N; j++) {
					if (i != j && interdiction[i][j] < 0.5 && interdiction[j][i] < 0.5 && Y[i][j] < 0.5) {
						Y[i][j] = Y[j][i] = 1.0;
						NumYOpened++;
						break;
					}
		...
		if (goIn == 0) {
			...
			startHere:
				...
				for (jj = ff; jj < kk; jj++) {
					for (j = 0; j < N; j++) {
						if (i != j) {
							if (Y[SubTree[jj]][j] > 0.5 && SubTree2[j] < 0.5) {
								SubTree2[j] = 1;
								SubTree[k] = j;
								k++;
								goIn = 1;
				...
				for (j = 0; j < N; j++) {
					if (i != j && SubTree2[j] < 0.5 && interdiction[i][j] < 0.5 && interdiction[j][i] < 0.5) {
						Y[i][j] = Y[j][i] = 1.0;
						NumYOpened++;
						break;
					}
...
void AMT(int N, double** C, double** W, double** Y, int** info) {
	...
	//////////////////////// if an arc is interdicted or the number of appearance in solution is more than MaxCountY, we allocate high cost to it
	for (i = 0; i < N - 1; i++) {
		for (j = i + 1; j < N; j++) {
			if (interdiction[i][j] == 1 || interdiction[i][j] == 1 || countY[i][j] > MaxCountY || countY[j][i] > MaxCountY) {
				costPrime[i][j] = M;
				costPrime[j][i] = M;
			}
			else {
	...
	for (i = 0; i < N; i++) { for (j = 0; j < N; j++) { dist_in_G[i][j] = costPrime[i][j]; } }
	...
	while (n_in_T < N) {	// While someone outside the tree:
		...
		for (i = 0; i < N; i++) {
			for (j = i + 1; j < N; j++) {
				if (in_T[i] != in_T[j] && (info == NULL || info[i][j] != NO)) {	// if the edge is allowed.
					delta = cc[i] + costPrime[i][j] * sum_w + cc[j];
		...
		if (in_T[best_i] == NO) {		// add best_i to the tree
			for (k = 0; k < N; k++) {
				if (in_T[k] == YES) {
					dist_in_T[k][best_i] = dist_in_T[k][best_j] + costPrime[best_j][best_i];
			...
			for (k = 0; k < N; k++) {
				if (in_T[k] == YES) {
					dist_in_T[k][best_j] = dist_in_T[k][best_i] + costPrime[best_i][best_j];
