...
int sampling(CONFIGURATION param, INSTANCE data, RESULTS* results) {
	...
	////// For each sample, we interdict the edges that are part of the y solution of previous sample with the lowest installation cost c_ij. Line 49-60 is finding the arcs with the lowest installation cost c_ij.
	...
	sampleIndex++;
	//gettimeofday(&startsample, NULL); /// time calculation in server ///
	startsample = clock();//

	for (sampleIndex = 1; sampleIndex < sample; sampleIndex++) {
		if (checktime < SampleTimeLimit) {
			...
			objSample[sampleIndex] = objective;
			counter = 0;
			for (e = 0; e < E; e++) {
				if (ySample[(index_i[e] * N + index_j[e]) + (sampleIndex * N * N)] > 0.01) {
					ySample[(index_i[e] * N + index_j[e]) + (sampleIndex * N * N)] = 1;
					Sorted[counter].index = e;
					Sorted[counter].value = data.C[index_i[e]][index_j[e]];
					counter++;
				}
				else {
					ySample[(index_i[e] * N + index_j[e]) + (sampleIndex * N * N)] = 0;
				}
