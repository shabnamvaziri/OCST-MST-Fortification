...
int solve_benders_subproblems(GLOBAL_INFO global, double *obj_values){
	...
	//double	available_time = MAX(0.0, global.param->MAX_CPU_TIME - elapsed(init_time_Linux)) / ((double)(K));
	///////////////////////////////////////////////////////////////////////////
	...
	for (e=0; e<E; e++) {
		...
		head[e] = index_j[e];
		head[e+E] = index_i[e];
		obj[e] = obj[e + E] = global.data->C[index_i[e]][index_j[e]];// +M * interdiction[index_i[e]][index_j[e]];
		lb[e] = lb[e+E] = 0.0;
	...
	for (k = 0; k < K; k++) {
		objective += global.data->Com[k].w * obj_values[k];
...
int solve_magnanti_wong_subproblems(GLOBAL_INFO global, double *obj_values, int flag) {
	...
	double	available_time = MAX(0.0, global.param->MAX_CPU_TIME - elapsed(global.var->init_time)) / ((double)(K));
	//double	available_time = MAX(0.0, global.param->MAX_CPU_TIME - elapsed(init_time_Linux)) / ((double)(K)); //activate for time calculation in server
	///////////////////////////////////////////////////////////////////////////
	...
	for (e=0; e<E; e++) {
		...
		head[e+E] = index_i[e];
		obj[e] = obj[e + E] = global.data->C[index_i[e]][index_j[e]];// +M * interdiction[index_i[e]][index_j[e]];
		//printf("interdiction[%d][%d]= %d\n", index_i[e], index_j[e], M * interdiction[index_i[e]][index_j[e]]);
		lb[e] = lb[e+E] = 0.0;
	...
	for (k=0; k<K; k++) {
		...
		if (solstat != CPX_STAT_OPTIMAL) { 
			...
			for (i=0; i<N; i++) { global.var->subproblem[k][E+i] = -beta[i]; }
			for (e=0; e<E; e++) {
				global.var->subproblem[k][e] = MAX(0, ABS(beta[index_i[e]] - beta[index_j[e]]) - (global.data->C[index_i[e]][index_j[e]]));// +M * interdiction[index_i[e]][index_j[e]]));
				if(global.var->subproblem[k][e] < 0.01) { global.var->subproblem[k][e] = 0; }
			}
		}
	}
