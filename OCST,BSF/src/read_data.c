...
extern int** interdiction, ** fortification, ** auxiliaryInterdiction, ** incumbantInterdiction, ** incumbantfortification, ** incumbantY, ** yAuxiliary, **interdictionperceived, ** waitingfortification, ** waitingInterdiction;
extern int Fbudget, Ibudget, tempN;
extern int sample, sampleIndex, bindex, iteration, iter, sampleRemoved, acceptedSample, initialsample;
extern double* obj, * ySample, zPerceived, UBound, LBound, * objSample, M, * waitinglistRealdamage, epsilonBSF;
...
// Select stopping criterions
	...
	param->MAX_CPU_TIME = 2 * 3600;	// In seconds
	...
	param->USE_ROUNDING_CALLBACK = NO;	// YES: Use the CPLEX Heuristic Callback with a rounding heuristic for frac solutions
	param->APPLY_AM_LOCAL_SEARCH = YES;	// YES:	Apply Ahuja-Murty Local search after rounding
	param->ADD_LOCAL_CUTS = NO;	// YES: Add local cuts in the callbacks!
...
int read_INSTANCE(INSTANCE* instance_ptr) {
	...
	Fbudget = 3;
	epsilonBSF = 0.25;
	interdiction = create_int_matrix(N, N);
	fortification = create_int_matrix(N, N);
	interdictionperceived = create_int_matrix(N, N);
	incumbantInterdiction = create_int_matrix(N, N);
	incumbantfortification = create_int_matrix(N, N);
	incumbantY = create_int_matrix(N, N);
	yAuxiliary = create_int_matrix(N, N);
	auxiliaryInterdiction = create_int_matrix(N * N, iteration);
	objSample = create_double_vector(sample + iteration);
	ySample = create_double_vector(N * N * (sample + iteration));
	waitinglistRealdamage = create_double_vector(iteration);
	waitingfortification = create_int_matrix(N * N, iteration);
	waitingInterdiction = create_int_matrix(N * N, iteration);
	...
	for (i = 0; i < N; i++) {
		for (j = 0; j < N; j++) {
			M += instance_ptr->C[i][j] * instance_ptr->W[i][j];
		}
	}

	M = M + 10;
	// Read the initial sample which is the solution of OCST without interdiction /////////////////////////////

	for (i = 0; i < N; i++) {
		for (j = 0; j < N; j++) {
			fscanf(input, "%lf", &ySample[(i * N + j) + (0 * N * N)]);
		}
	}
	// Read the objective of the initial sample /////////////////////////////
	fscanf(input, "%lf ", &objSample[0]);
...
INSTANCE* create_INSTANCE_vector(int* cells_ptr, const char* filename) {
	...
	fscanf(instance_list, "%s", &outfile);
...
void free_INSTANCE(INSTANCE* instance_ptr) {
	...
	free(interdiction);
	free(fortification);
	free(interdictionperceived);
	free(incumbantInterdiction);
	free(incumbantfortification);
	free(incumbantY);
	free(yAuxiliary);
	free(auxiliaryInterdiction);
	free(objSample);
	free(ySample);
	free(waitingfortification);
	free(waitingInterdiction);
	free(waitinglistRealdamage);
}
