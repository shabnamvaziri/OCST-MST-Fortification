...
double LongestHamiltonianPathUB(int N, double** C) {
	...
	row_max = C[0][0];
	for (col = 1; col < N; col++) { row_max = MAX(row_max, (C[0][col])); }// + M * interdiction[0][col])); }
	min_row_max = row_max;
	UB = row_max;
	...
	for (row = 1; row < N; row++) {
		row_max = C[row][0];
		for (col = 1; col < N; col++) { row_max = MAX(row_max, (C[row][col])); } //+ M * interdiction[row][col])); }
...
void GH_tree(int N, double** Capacity, int* Tree, double* Flow) {
	...
	for (s = 1; s < N; s++) {                // the '0' node is already 'visited'
		...
		checkVisted = 0;
		for (i = 0; i < N; i++) {
			if (in_s_side[i] == YES && i != s && Tree[i] == t && interdiction[i][s] < 1 && interdiction[s][i] < 1) {
				Tree[i] = s;
				checkVisted = 1;
				/*printf("Tree[%d]=%d\n", i, Tree[i]);*/
			}
		...
		if (in_s_side[Tree[t]] == YES && interdiction[Tree[t]][s] < 1 && interdiction[s][Tree[t]] < 1 && interdiction[t][s] < 1 && interdiction[s][t] < 1) {
			Tree[s] = Tree[t];
			Tree[t] = s;
			auxFlow = Flow[s];
			Flow[s] = Flow[t];
	...
	/*if (checkVisted == 0) {
		printf("HEEEEEEERRREEEE!\n");
	}*/
	/*for (s = 0; s < N - 1; s++) {
		for (t = s + 1; t < N; t++) {
			if (interdiction[s][t] == 1 || interdiction[t][s] == 1) {
				if (Tree[s] == t) {
					if (t + 1 != N-1) {
						if (interdiction[s][t + 1] == 0 && interdiction[t + 1][s] == 0) {
							Tree[s] = t + 1;
						}
						else if (interdiction[s][t - 1] == 0 && interdiction[t - 1][s] == 0) {
							Tree[s] = t - 1;
						}
					}
					else if (t + 1 == N-1) {
					...
					if (s + 1 != N-1) {
						if (interdiction[s + 1][t] == 0 && interdiction[t][s + 1] == 0) {
							Tree[t] = s + 1;
						}
						else if (interdiction[s + 2][t] == 0 && interdiction[t][s + 2] == 0) {
							Tree[t] = s + 2;
						}
					}
					else if (s + 1 == N-1) {
						if (interdiction[s-1][t] == 0 && interdiction[t][s - 1] == 0) {
							if (Tree[t] == s - 1 || Tree[s - 1] == t) {
								Tree[t] = s - 2;
							}
							else {
								Tree[t] = s - 1;
...
double maxFlowAndCut(int N, int s, int t, double** Capacity, int* visited) {
	...
	while (flow > 0.0) {
		...
		while (queueIndex < queueSize) {
			...
			for (v = 0; v < N; v++) {
				/*printf("interdiction[%d][%d]=%d\n", u, v, interdiction[u][v]);
				printf("interdiction[%d][%d]=%d\n", v, u, interdiction[v][u]);*/

				//if (interdiction[u][v] == 1 || interdiction[v][u] == 1) {////////////////////////////////////////////////////////////////////////////////////////////
				//	/*printf("interdiction[%d][%d]=%d\n", u, v, interdiction[u][v]);
				//	printf("interdiction[%d][%d]=%d\n", v, u, interdiction[v][u]);*/
				//	currentFlow[u][v] = Capacity[u][v];
				//}

				if (visited[v] == NO && Capacity[u][v] - currentFlow[u][v] > 0 && interdiction[u][v] < 1 && interdiction[v][u] < 1) {
	...
	for (i = 0; i < N; i++) {
		...
		while (size > 0) {
			...
			for (d = 0; d < degree[j]; d++) {
				k = neighbours[j][d];
				if (dist[i][k] == -1.0) {
					dist[i][k] = dist[i][j] + C[j][k];// +M * interdiction[j][k];
					stack[size++] = k;
				}
			}
		}
