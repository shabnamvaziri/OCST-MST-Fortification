...
extern int M, sample, sampleIndex;
...
int Heuristics(CONFIGURATION param, INSTANCE data, RESULTS* results) {
	...
	start = clock();
	for (k = 0; k < K; k++) {
		results->LowerBound += Com[k].w * (C1[Com[k].o][Com[k].d]);// +M * interdiction[Com[k].o][Com[k].d]);
		//printf("interdiction[%d][%d]= %d\n", Com[k].o, Com[k].d, interdiction[Com[k].o][Com[k].d]);
	}
	...
	upp = 0.0;
	for (k = 0; k < K; k++) { upp += Com[k].w * (dist[Com[k].o][Com[k].d]); }// +M * interdiction[Com[k].o][Com[k].d]); }
	...
	for (i = 0; i < N; i++) {
		for (j = i + 1; j < N; j++) {
			...
			//printf("yheuristic[%d][%d]=%lf\n", i, j, Y[i][j]);
		}
	}
	for (k = 0; k < K; k++) { results->best_solution[E + k] = dist[Com[k].o][Com[k].d]; }// + M * interdiction[Com[k].o][Com[k].d];
