...
void MST(int N, double** cost, double** arc, int** info) {
	...
	for (i = 0; i < N; i++) { pred[i] = 0; }			// and have 0 as a predecesor
	for (i = 0; i < N; i++) {
		/*if (interdiction[0][i] == 1 || interdiction[i][0] == 1) {
			dist[i] = M;
		}
		else {*/
	...
	//if (iter == 122) {
		//printf("\niter\t=\t%d\npred:______________________________:\n",iter);
		//for (i = 0; i < N; i++) {
		//	printf("pred[%d]\t=\t%d\n", i, pred[i]);
		//}
	//}
	...
	for (i = 0; i < N; i++) {
		for (j = i + 1; j < N; j++) {
			if (arc[i][j] > 0) {
				//printf("arc[%d][%d]\t=\t%.1f\t%.1f\n", i, j, arc[i][j], costPrime[i][j]);
				//if (interdiction[i][j] == 1 || interdiction[j][i] == 1) {
				//	printf("here\t");
				//}
			}
		}
...
void GHT(int N, double** W, double** Y) {
	...
	for (i = 0; i < N; i++) {		// capacity must be symmetric :-/
		...
		for (j = i + 1; j < N; j++) {
			...
			//printf("capacity[%d][%d]=%lf\n", i, j, capacity[i][j]);
		}
	}

	/*for (j = i + 1; j < N; j++) {
		if (interdiction[i][j] == 1) {
