...
extern int** interdiction, ** fortification, ** auxiliaryInterdiction, ** incumbantInterdiction, ** incumbantfortification, ** incumbantY, ** yAuxiliary, ** interdictionperceived, ** InterdictionS, ** waitingfortification, ** waitingInterdiction;
extern int Fbudget, *Ibudget, tempN, S, sglobal;
extern int sample, sampleIndex, bindex, iteration, iter, sampleRemoved, acceptedSample, initialsample;
extern double* obj, * ySample, zPerceived, * objSample, M, *p, * waitinglistRealdamage, epsilonBSF;
...
int read_INSTANCE(INSTANCE* instance_ptr) {
	...
	incumbantInterdiction = create_int_matrix(N * N, S);
	InterdictionS = create_int_matrix(N * N, S);
	incumbantfortification = create_int_matrix(N, N);
	incumbantY = create_int_matrix(N * N, S);
	yAuxiliary = create_int_matrix(N * N, S);
	auxiliaryInterdiction = create_int_matrix(N * N, iteration * S);
	...
	waitingfortification = create_int_matrix(N * N * S, iteration);
	waitingInterdiction = create_int_matrix(N * N * S, iteration);
	...
	for (i = 0; i < N; i++) {
		for (j = 0; j < N; j++) {
			if (M < instance_ptr->C[i][j]) {
				M = instance_ptr->C[i][j];
			}
		}
	}
	M = M + 2;
