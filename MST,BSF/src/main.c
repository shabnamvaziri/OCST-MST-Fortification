...
double* objSample, * ySample, zPerceived, UBound, LBound, zbar, M, * waitinglistRealdamage;
int checkVisted, tempN, globalStop, waitinglist, indexGlobal, flag;
double incumbantzbar, incumbantUB, incumbantLB, ratio;
...
int main(int argc, char** argv) {
	...
	int i, e, ii, bindex, j, index1, index, k;						// Index variable
	...
	if (argc < 3) { show_help(argv[0]); }
		...
		for (i = 0; i < n_of_instances; i++) {
			...
			startTotal = clock();//
			startSampling = clock();//
			//gettimeofday(&startTotal, NULL); //For time calculation in server
			...
			while (TimeBSF < Timelimit && globalStop == 0 && sampleIndex < iteration) {
				while (Fortification_Flag == 0 && TimeBSF < Timelimit && sampleIndex < iteration) {
					...
					while (LBound < zbar && TimeBSF < Timelimit && sampleIndex < iteration) {
						...
						/// when epsilon = 0, the algorithm reduces to backward sampling framework without acceleration technique
						...
						printf("sample = %d \t LB = %.3f \t UB =  %.3f \t Zbar =  %.3f/n", sampleIndex, LBound, UBound, zbar);
					}
				}
